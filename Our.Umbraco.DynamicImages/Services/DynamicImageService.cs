...
namespace Our.Umbraco.DynamicImages.Services
{
    ...
    public class DynamicImageService : IDynamicImageService
    {
        ...
        public Udi GenerateImageAsMediaItem(DynamicImageSettings settings, int parentFolderId, string umbracoMediaAlias, string imageName)
        {
            byte[] photoBytes = GetImageBytesFromUrl(settings.BackgroundImageUrl);
            ISupportedImageFormat format = new PngFormat() { Quality = settings.BackgroundImageQuality };
            using (MemoryStream inStream = new MemoryStream(photoBytes))
        ...
        private void ApplyLayersToMemoryStream(DynamicImageSettings settings, ISupportedImageFormat format, MemoryStream inStream, MemoryStream outStream)
        {
